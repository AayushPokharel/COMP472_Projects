...
@app.route("/solution")
def solution():
    # Generate both BFS and DFS trees
    bfs_tree = generate_search_tree("bfs")
    dfs_tree = generate_search_tree("dfs")

    # Convert the tree data to SVG format using pydot
    bfs_graph = pydot.Dot(graph_type="graph")
    dfs_graph = pydot.Dot(graph_type="graph")
    add_edges(bfs_graph, bfs_tree)
    add_edges(dfs_graph, dfs_tree)
    bfs_svg_string = bfs_graph.create_svg().decode("utf-8")
    dfs_svg_string = dfs_graph.create_svg().decode("utf-8")

    return render_template(
        "solution.html",
        bfs_svg_string=bfs_svg_string,
        dfs_svg_string=dfs_svg_string,
    )
